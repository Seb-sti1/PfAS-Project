...
def draw_registrations(cloud, transformation=None):
...
def depth_to_pcd(image_left: ndarray, disparity: ndarray, scale: float = 1) -> o3d.geometry.PointCloud:
    ...
    pcd.points = o3d.utility.Vector3dVector(points_3d[mask_map])
...
def main(sequence):
    ...
    for i, (rec_left, rec_right, disparity, pcd) in enumerate(get_stereo_image_disparity_pcd(sequence, stereo)):
        cv2.imshow("rec", cv2.resize(disparity / disparity.max(), None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR))
        draw_registrations(pcd)
