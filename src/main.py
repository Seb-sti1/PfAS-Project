...
def custom_imshow(title, image, resize=0.6):
    cv2.imshow(title, cv2.resize(image, None,
                                 fx=resize, fy=resize, interpolation=cv2.INTER_LINEAR))
...
def everything_homemade(seq):
    ...
    # display params
    labels_pd = load_labels("rec_data", seq) if seq != "seq_03" else None
    colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
    name_to_index = {"Pedestrian": 0, "Cyclist": 1, "Car": 2}

    for index, ((raw_left, raw_right), (true_rec_left, true_rec_right)) in enumerate(
            zip(load_stereo_images("raw_data", seq),
        ...
        custom_imshow("raw_left", raw_left)

        yolo_rec_left = rec_left.copy()
        for pred in raw_measured_roi_list:
            draw_rectangle(yolo_rec_left, *pred, colors)
        custom_imshow("yolo detection on rec_left", yolo_rec_left)

        custom_imshow("disparity", disparity / disparity.max())

        for pred in prediction_list:
            draw_circle(rec_left, *pred, colors)
        custom_imshow("rec_left", rec_left, resize=1)

        if labels_pd is not None:
            current_labels = labels_pd[labels_pd["frame"] == index]
            add_labels(true_rec_left, current_labels, {name: colors[value] for name, value in name_to_index.items()})
        custom_imshow("true_rec_left", true_rec_left, resize=1)
...
def main(args=None):
    ...
    everything_homemade(args.sequence)
